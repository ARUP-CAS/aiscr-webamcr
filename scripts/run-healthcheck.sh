...
test "${status_code}" -eq "200" && "${fedora_status_code}" -eq "200" && python /scripts/db/db_connection_from_docker-web.py && test ${disk_status} -eq 0 && test ${ram_status} -eq 0 && true || false
