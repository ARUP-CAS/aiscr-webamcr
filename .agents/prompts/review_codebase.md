...
4. **Datetime values** (e.g. `completed_at`, `generated_at`, `last_updated`, `**Datum**` in reports) MUST be set from a verifiable source, not guessed. Use either:
   - **Git:** for an existing file, run `git log -1 --format=%ci -- <path>` and convert to ISO 8601 (e.g. use the commit date as the completion/generation time for that artefact); or
   - **Python:** when creating or updating a file, use e.g. `datetime.datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%SZ')` (or equivalent) so the timestamp reflects the actual write time.
   Do not invent or guess timestamps.
...
The Django project root is `webclient/`; `manage.py` and `requirements.txt` are located there, not in the repository root.
