...
Long-running codebase review for the repository it runs in (any AIS CR repo except the management hub); two modes (full pass T01-T11, incremental update U01-U06) dispatched via detect -> propose-with-evidence -> user-confirm. Owns the cache and report schema, severity vocabulary, and English-default output language; repo-specific scope stays in the target repo's review_config.toml. The generated skill embeds its runbook.
