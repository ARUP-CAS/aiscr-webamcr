...
.. py:function:: get_or_create_transaction(instance)
   ...
   **Parametry:**
   ...
   - ``instance``: Parametr ``instance`` pracuje se s atributy ``active_transaction``, ovlivňuje větvení podmínek, vstupuje do návratové hodnoty.

   **Návratová hodnota:**

   Vrací hodnotu podle větve zpracování, typicky: atribut objektu, výsledek volání ``FedoraTransaction()``.
...
.. py:function:: save_ident_cely(sender, instance)
   ...
   **Parametry:**
   ...
   - ``sender``: Model třídy, která poslala signál.
   - ``instance``: Parametr ``instance`` předává se do volání ``check_container_deleted_or_not_exists()``, ``debug()``, pracuje se s atributy ``ident_cely``, ``pk``, ovlivňuje větvení podmínek.
   - ``kwargs``: Další klíčové argumenty signálu.

   **Výjimky:**

   - ``Exception``: Vyvolá se při splnění podmínky ``not FedoraRepositoryConnector.check_container_deleted_or_not_exists(instance.ident_cely, 'heslar')``.
...
.. py:function:: save_metadata_heslar(sender, instance)
   ...
   **Parametry:**
   ...
   - ``sender``: Model třídy, která poslala signál.
   - ``instance``: Parametr ``instance`` předává se do volání ``on_commit()``, pracuje se s atributy ``suppress_signal``, ovlivňuje větvení podmínek.
   - ``kwargs``: Další klíčové argumenty signálu.
...
.. py:function:: save_metadata_heslar_hierarchie(sender, instance, created)
   ...
   **Parametry:**
   ...
   - ``sender``: Model třídy, která poslala signál.
   - ``instance``: Parametr ``instance`` pracuje se s atributy ``suppress_signal``, ovlivňuje větvení podmínek.
   - ``created``: Parametr ``created`` slouží jako vstup pro logiku funkce ``save_metadata_heslar_hierarchie``.
   - ``kwargs``: Další klíčové argumenty signálu.
...
.. py:function:: save_metadata_heslar_datace(sender, instance, created)
   ...
   **Parametry:**
   ...
   - ``sender``: Model třídy, která poslala signál.
   - ``instance``: Parametr ``instance`` pracuje se s atributy ``suppress_signal``, ovlivňuje větvení podmínek.
   - ``created``: Parametr ``created`` slouží jako vstup pro logiku funkce ``save_metadata_heslar_datace``.
   - ``kwargs``: Další klíčové argumenty signálu.
...
.. py:function:: save_metadata_heslar_odkaz(sender, instance, created)
   ...
   **Parametry:**
   ...
   - ``sender``: Model třídy, která poslala signál.
   - ``instance``: Parametr ``instance`` pracuje se s atributy ``suppress_signal``, ovlivňuje větvení podmínek.
   - ``created``: Parametr ``created`` slouží jako vstup pro logiku funkce ``save_metadata_heslar_odkaz``.
   - ``kwargs``: Další klíčové argumenty signálu.
...
.. py:function:: heslar_delete_repository_container(sender, instance)
   ...
   **Parametry:**
   ...
   - ``sender``: Model třídy, která poslala signál.
   - ``instance``: Parametr ``instance`` předává se do volání ``on_commit()``.
   - ``kwargs``: Další klíčové argumenty signálu.
...
.. py:function:: ruian_katastr_delete_repository_container(sender, instance)
   ...
   **Parametry:**
   ...
   - ``sender``: Model třídy, která poslala signál.
   - ``instance``: Parametr ``instance`` předává se do volání ``get_or_create_transaction()``, ``on_commit()``.
   - ``kwargs``: Další klíčové argumenty signálu.
...
.. py:function:: ruian_okres_delete_repository_container(sender, instance)
   ...
   **Parametry:**
   ...
   - ``sender``: Model třídy, která poslala signál.
   - ``instance``: Instance modelu, která byla uložena.
   - ``kwargs``: Další klíčové argumenty signálu.
...
.. py:function:: delete_uppdate_related_heslar_datace(sender, instance)
   ...
   **Parametry:**
   ...
   - ``sender``: Model třídy, která poslala signál.
   - ``instance``: Parametr ``instance`` pracuje se s atributy ``obdobi``.
   - ``kwargs``: Další klíčové argumenty signálu.
