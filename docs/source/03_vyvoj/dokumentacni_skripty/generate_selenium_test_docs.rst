...
.. py:function:: _repo_root_from_script()
   ...
   **Návratová hodnota:**
   ...
   Vrací hodnotu typu ``Path`` (vybranou hodnotu z kolekce).
...
.. py:function:: _is_ignored_path(p)
   ...
   **Parametry:**
   ...
   - ``p``: Parametr ``p`` předává se do volání ``set()``, pracuje se s atributy ``parts``.

   **Návratová hodnota:**

   Vrací hodnotu typu ``bool`` (výsledek volání ``any()``).
...
.. py:function:: _find_rst_file(root)
   ...
   **Parametry:**
   ...
   - ``root``: Parametr ``root`` pracuje se s atributy ``rglob``.

   **Návratová hodnota:**

   Vrací hodnotu typu ``Path``; podle větve může jít o: proměnná ``path``, proměnná ``c``.

   **Výjimky:**

   - ``FileNotFoundError``: Vyvolá se s textem "Nenalezen soubor selenium_testy.rst (zkontroluj umístění v repozitáři).".
...
.. py:function:: _iter_test_files(root)
   ...
   **Návratová hodnota:**
   ...
   Vrací hodnotu typu ``List[Path]`` (výsledek volání ``sorted()``).
...
.. py:function:: _get_app_name(file_path)
   ...
   **Parametry:**
   ...
   - ``file_path``: Parametr ``file_path`` předává se do volání ``list()``, pracuje se s atributy ``parts``.

   **Návratová hodnota:**

   Vrací hodnotu typu ``str``; podle větve může jít o: str, vybranou hodnotu z kolekce.
...
.. py:function:: _module_dotted(root, file_path)
   ...
   **Parametry:**
   ...
   - ``root``: Parametr ``root`` předává se do volání ``relative_to()``.
   - ``file_path``: Parametr ``file_path`` pracuje se s atributy ``relative_to``.

   **Návratová hodnota:**

   Vrací hodnotu typu ``str`` (výsledek volání ``join()``).
...
.. py:function:: _extract_test_no(name)
   ...
   **Parametry:**
   ...
   - ``name``: Parametr ``name`` předává se do volání ``match()``.

   **Návratová hodnota:**

   Vrací hodnotu typu ``Optional[int]`` (hodnotu podle větve zpracování).
...
.. py:function:: _split_summary_and_rest(doc)
   ...
   **Parametry:**
   ...
   - ``doc``: Parametr ``doc`` předává se do volání ``dedent()``.

   **Návratová hodnota:**

   Vrací hodnotu typu ``Tuple[str, str]`` (n-tici).
...
.. py:function:: _parse_description_and_sections(rest)
   ...
   **Parametry:**
   ...
   - ``rest``: Parametr ``rest`` pracuje se s atributy ``strip``, ``splitlines``, ovlivňuje větvení podmínek.

   **Návratová hodnota:**

   Vrací hodnotu typu ``Tuple[str, Dict[str, str]]`` (n-tici).
...
.. py:function:: _summary_title_or_error(summary, test_no, origin)
   ...
   **Parametry:**
   ...
   - ``summary``: Parametr ``summary`` předává se do volání ``match()``.
   - ``test_no``: Parametr ``test_no`` předává se do volání ``ValueError()``, ovlivňuje větvení podmínek.
   - ``origin``: Parametr ``origin`` předává se do volání ``ValueError()``.

   **Návratová hodnota:**

   Vrací hodnotu typu ``str`` (proměnná ``rest``).

   **Výjimky:**

   - ``ValueError``: Vyvolá se při splnění podmínky ``not m``; nebo při splnění podmínky ``not rest``.
...
.. py:function:: _validate_unique_test_numbers(all_docs)
   ...
   **Parametry:**
   ...
   - ``all_docs``: Parametr ``all_docs`` slouží jako vstup pro logiku funkce ``_validate_unique_test_numbers``.

   **Návratová hodnota:**

   Vrací hodnotu typu ``List[str]`` (proměnná ``errs``).
...
.. py:function:: _validate(origin, summary, sections, has_docstring, test_no)
   ...
   **Parametry:**
   ...
   - ``origin``: Parametr ``origin`` předává se do volání ``append()``, ``_summary_title_or_error()``.
   - ``summary``: Parametr ``summary`` předává se do volání ``_summary_title_or_error()``, pracuje se s atributy ``strip``, ovlivňuje větvení podmínek.
   - ``sections``: Parametr ``sections`` ovlivňuje větvení podmínek.
   - ``has_docstring``: Parametr ``has_docstring`` ovlivňuje větvení podmínek.
   - ``test_no``: Parametr ``test_no`` předává se do volání ``_summary_title_or_error()``.
...
.. py:function:: _rst_title(text, underline)
   ...
   **Parametry:**
   ...
   - ``text``: Parametr ``text`` předává se do volání ``len()``, vstupuje do návratové hodnoty.
   - ``underline``: Parametr ``underline`` vstupuje do návratové hodnoty.

   **Návratová hodnota:**

   Vrací hodnotu typu ``str`` (hodnotu podle větve zpracování).
...
.. py:function:: _app_heading(app)
   ...
   **Parametry:**
   ...
   - ``app``: Parametr ``app`` předává se do volání ``_rst_title()``, pracuje se s atributy ``replace``, vstupuje do návratové hodnoty.

   **Návratová hodnota:**

   Vrací hodnotu typu ``str`` (výsledek volání ``_rst_title()``).
...
.. py:function:: _test_anchor(app, test_no, fallback_name)
   ...
   **Parametry:**
   ...
   - ``app``: Parametr ``app`` předává se do volání ``sub()``, pracuje se s atributy ``lower``.
   - ``test_no``: Parametr ``test_no`` slouží jako vstup pro logiku funkce ``_test_anchor``.
   - ``fallback_name``: Parametr ``fallback_name`` předává se do volání ``sub()``, pracuje se s atributy ``lower``.

   **Návratová hodnota:**

   Vrací hodnotu typu ``str`` (výsledek volání ``rstrip()``).
...
.. py:function:: _first_nonempty_line(s)
   ...
   **Parametry:**
   ...
   - ``s``: Parametr ``s`` slouží jako vstup pro logiku funkce ``_first_nonempty_line``.

   **Návratová hodnota:**

   Vrací hodnotu typu ``str``; podle větve může jít o: výsledek volání ``strip()``, str.
...
.. py:function:: _role_short(sections)
   ...
   **Parametry:**
   ...
   - ``sections``: Parametr ``sections`` předává se do volání ``_first_nonempty_line()``, pracuje se s atributy ``get``, vstupuje do návratové hodnoty.

   **Návratová hodnota:**

   Vrací hodnotu typu ``str`` (výsledek volání ``_first_nonempty_line()``).
...
.. py:function:: _popis_short(description)
   ...
   **Parametry:**
   ...
   - ``description``: Parametr ``description`` předává se do volání ``_first_nonempty_line()``.

   **Návratová hodnota:**

   Vrací hodnotu typu ``str``; podle větve může jít o: hodnotu podle větve zpracování, proměnná ``line``.
...
.. py:function:: _render_detail_for_test(t)
   ...
   **Parametry:**
   ...
   - ``t``: Parametr ``t`` předává se do volání ``append()``, ``_rst_title()``, pracuje se s atributy ``anchor``, ``summary``, ovlivňuje větvení podmínek.
...
.. py:function:: _render_summary_table(all_docs)
   ...
   **Parametry:**
   ...
   - ``all_docs``: Parametr ``all_docs`` předává se do volání ``sorted()``.
...
.. py:function:: _render(all_docs)
   ...
   **Parametry:**
   ...
   - ``all_docs``: Parametr ``all_docs`` slouží jako vstup pro logiku funkce ``_render``.
...
.. py:function:: _replace_autoblock(original, generated)
   ...
   **Parametry:**
   ...
   - ``original``: Parametr ``original`` pracuje se s atributy ``split``, ovlivňuje větvení podmínek.
   - ``generated``: Parametr ``generated`` pracuje se s atributy ``rstrip``.
   ...
   **Výjimky:**
   ...
   - ``RuntimeError``: Vyvolá se při splnění podmínky ``START_MARKER not in original or END_MARKER not in original``.
...
.. py:function:: main()
   ...
   **Návratová hodnota:**
   ...
   Návratový kód procesu generování dokumentace selenium testů.
