...
from pas.api import ImportErrorType, SamostatnyNalezXmlImportView
...
class SamostatnyNalezXmlImportViewTests(TestCase):
    ...
    def setUp(self):
        """Mock volání Fedora repozitáře pro každý test."""
    ...
    def _assert_xml_success_response(self, response, ident_cely: str) -> None:
        ...
        """
        self.assertEqual(response.status_code, SamostatnyNalezXmlImportView.HTTP_200_OK)
        self.assertEqual(response["Content-Type"], "application/xml")
        self.assertIn(ident_cely, response.content.decode("utf-8"))

    def test_missing_file_returns_400(self):
        """POST bez souboru vrátí HTTP 400."""
        client = APIClient()
        client.credentials(HTTP_AUTHORIZATION=f"Bearer {self.token.key}")
        response = client.post(self.xml_url, {}, format="multipart")
        self.assertEqual(response.status_code, SamostatnyNalezXmlImportView.HTTP_400_BAD_REQUEST)
    ...
    def test_wrong_amcr_namespace_version_returns_422(self):
        ...
        self.assertEqual(response.status_code, SamostatnyNalezXmlImportView.HTTP_422_UNPROCESSABLE_ENTITY)
        self.assertIn("detail", response.data)
    ...
    def test_unauthenticated_returns_401(self):
        """Požadavek bez tokenu vrátí HTTP 401."""
        client = APIClient()
    ...
    def test_invalid_token_returns_401(self):
        """Požadavek s neplatným tokenem vrátí HTTP 401."""
        response = self._post_xml(b"<x/>", token="neplatnytoken")
        self.assertEqual(response.status_code, 401)

    def test_get_method_not_allowed(self):
        """GET požadavek na XML endpoint vrátí HTTP 405."""
        client = APIClient()
        client.credentials(HTTP_AUTHORIZATION=f"Bearer {self.token.key}")
    ...
    def test_user_without_project_permission_returns_403(self):
        ...
        self.assertEqual(response.status_code, SamostatnyNalezXmlImportView.HTTP_403_FORBIDDEN)
        self.assertIn("detail", response.data)
    ...
    def test_nonexistent_nalezce_returns_404(self):
        ...
        self.assertEqual(response.status_code, SamostatnyNalezXmlImportView.HTTP_404_NOT_FOUND)
        self.assertIn("validation_errors", response.data)
