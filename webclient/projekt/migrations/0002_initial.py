# Generated by Django 3.2.11 on 2023-02-14 19:39

from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):

    """Zapouzdřuje chování třídy ``Migration`` pro modul ``webclient.projekt.migrations.0002_initial``."""
    initial = True

    dependencies = [
        ('projekt', '0001_initial'),
        ('heslar', '0002_initial'),
        ('core', '0002_initial'),
        ('uzivatel', '0001_initial'),
    ]

    operations = [
        migrations.AddField(
            model_name='projekt',
            name='organizace',
            field=models.ForeignKey(blank=True, db_column='organizace', null=True, on_delete=django.db.models.deletion.RESTRICT, to='uzivatel.organizace'),
        ),
        migrations.AddField(
            model_name='projekt',
            name='soubory',
            field=models.OneToOneField(blank=True, db_column='soubory', null=True, on_delete=django.db.models.deletion.SET_NULL, related_name='projekt_souboru', to='core.souborvazby'),
        ),
        migrations.AddField(
            model_name='projekt',
            name='typ_projektu',
            field=models.ForeignKey(db_column='typ_projektu', limit_choices_to={'nazev_heslare': 41}, on_delete=django.db.models.deletion.RESTRICT, related_name='projekty_typu', to='heslar.heslar', verbose_name='Typ projektu'),
        ),
        migrations.AddField(
            model_name='projekt',
            name='vedouci_projektu',
            field=models.ForeignKey(blank=True, db_column='vedouci_projektu', null=True, on_delete=django.db.models.deletion.RESTRICT, to='uzivatel.osoba', verbose_name='Vedoucí projektů'),
        ),
        migrations.AlterUniqueTogether(
            name='projektkatastr',
            unique_together={('projekt', 'katastr')},
        ),
    ]
