...
class AkceDokumenty(BaseSeleniumTestClass):
    ...
    def test_141_test_Fedory_dokument_p_001(self):
        ...
        time = self.getTime()
        self.select_nth_selectpicker_option("id_modal-region")
        self.ElementClick(By.ID, "submit-btn")
        ...
        self.select_dynamic_selectpicker_option("id_typ_dokumentu", "letecká fotografie – lokalita", wait_ajax=True)
        self.select_nth_selectpicker_option("id_material_originalu")
        # self.wait(1)
        self.ElementClick(By.ID, "id_popis")
        ...
        time = self.getTime()
        self.ElementClick(By.ID, "edit-btn")
        self.select_dynamic_selectpicker_option("id_let", "C-LET-00001")
        with WaitForPageLoad(self.driver):
            self.ElementClick(By.ID, "newDocumentSubmitBtn")
        ...
        self.select_dynamic_select2_autocomplete_option("id_arch_z", "C-200810821A")
        with WaitForPageLoad(self.driver):
            self.ElementClick(By.ID, "submit-btn")
        ...
        self.select_dynamic_select2_autocomplete_option("id_arch_z", "C-K9000001")
        with WaitForPageLoad(self.driver):
            self.ElementClick(By.ID, "submit-btn")
        ...
        self.select_dynamic_select2_autocomplete_option("id_projekt", "C-201911202")
        with WaitForPageLoad(self.driver):
            self.ElementClick(By.ID, "submit-btn")
        ...
        self.select_dynamic_selectpicker_option("id_obdobi", "únětická kultura")
        self.select_nth_selectpicker_option("id_jistota")
        self.select_dynamic_selectpicker_option("id_areal", "pohřební areál")
        self.select_dynamic_selectpicker_option("id_aktivity", "pohřbívání")
        with WaitForPageLoad(self.driver):
        ...
        time = self.getTime()
        self.select_dynamic_selectpicker_option(f"id_{ident}-K001_o-0-druh", "kašna")
        self.driver.find_element(By.ID, f"id_{ident}-K001_o-0-pocet").send_keys("1")
        self.select_dynamic_selectpicker_option(f"id_{ident}-K001_p-0-druh", "hudební nástroj", wait_ajax=True)
        self.select_dynamic_selectpicker_option(f"id_{ident}-K001_p-0-specifikace", "dehet")
        with WaitForPageLoad(self.driver):
        ...
        # Vytvoření tvaru
        time = self.getTime()
        self.select_dynamic_selectpicker_option(f"id_{ident}_d-0-tvar", "kruhové ohrazení (malé)")
        with WaitForPageLoad(self.driver):
...
class AkceKnihovna3D(BaseSeleniumTestClass):
    ...
    def zapsat_zaznam(self):
        ...
        """
        self.go_to_form_zapsat()
        self.select_multiple_select2_autocomplete_options("id_autori", ["švejcar"])
        ...
        self.select_dynamic_selectpicker_option("id_organizace", "Archeologický ústav Praha", wait_ajax=True)
        self.select_dynamic_selectpicker_option("id_typ_dokumentu", "3D – rekonstrukce")
        self.select_nth_selectpicker_option("id_licence")
        with WaitForPageLoad(self.driver):
            self.ElementClick(By.ID, "newDocumentSubmitBtn")
        ident = self.driver.current_url.split("/")[-1]
    ...
    def test_144_test_Fedory_3D_p_001(self):
        ...
        self.select_nth_selectpicker_option("id_format")
        with WaitForPageLoad(self.driver):
            self.ElementClick(By.ID, "newDocumentSubmitBtn")
        ...
        time = self.getTime()
        self.ElementClick(By.ID, "buttonEdit")
        self.select_dynamic_selectpicker_option("id_komponenta-obdobi", "paleolit")
        self.select_dynamic_selectpicker_option("id_komponenta-areal", "sídliště", wait_ajax=True)
        self.ElementClick(By.ID, "id_duveryhodnost")
        ...
        # Vytvoření nálezu
        time = self.getTime()
        self.select_dynamic_selectpicker_option(f"id_{ident}-K001_o-0-druh", "budova/stavba", wait_ajax=True)
        self.select_dynamic_selectpicker_option(f"id_{ident}-K001_o-0-specifikace", "konstrukce – zděná")
        self.ElementClick(By.ID, f"id_{ident}-K001_o-0-pocet")
        self.driver.find_element(By.ID, f"id_{ident}-K001_o-0-pocet").send_keys("1")
        self.ElementClick(By.ID, f"id_{ident}-K001_o-0-poznamka")
        self.driver.find_element(By.ID, f"id_{ident}-K001_o-0-poznamka").send_keys("test")
        self.select_dynamic_selectpicker_option(f"id_{ident}-K001_p-0-druh", "zlomek mazanice", wait_ajax=True)
        self.select_dynamic_selectpicker_option(f"id_{ident}-K001_p-0-specifikace", "malta")
        self.ElementClick(By.ID, f"id_{ident}-K001_p-0-pocet")
        self.driver.find_element(By.ID, f"id_{ident}-K001_p-0-pocet").send_keys("2")
        self.ElementClick(By.ID, f"id_{ident}-K001_p-0-poznamka")
        self.driver.find_element(By.ID, f"id_{ident}-K001_p-0-poznamka").send_keys("test")
