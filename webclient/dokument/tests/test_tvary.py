...
def _dokument(typ_dokumentu_id, rada_zkratka):
    """
    Sestaví náhradu za dokument s poli, na která se funkce dotazuje.

    :param typ_dokumentu_id: Identifikátor typu dokumentu.
    :param rada_zkratka: Zkratka řady dokumentu, nebo ``None`` pro dokument bez řady.
    :return: Mock dokumentu.
    """
    rada = None if rada_zkratka is None else mock.Mock(zkratka=rada_zkratka)
    return mock.Mock(typ_dokumentu_id=typ_dokumentu_id, rada=rada)
