...
def get_dokument_soubor_name(dokument: Dokument, filename: str, add_to_index=1):
    ...
    Název se přiděluje navýšením podle nejvyššího obsazeného písmenného suffixu (``A`` … ``Z``). Toto
    výchozí chování se záměrně nemění – uvolnění či změnu pozice řeší přejmenování souboru.
