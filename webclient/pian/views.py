...
class PianPermissionFilterMixin(PermissionFilterMixin):
    def filter_by_permission(self, qs, permission):
        qs = qs.annotate(
            ...
            historie_zapsat_az=FilteredRelation(
                ...
                condition=Q(
                    **{"dokumentacni_jednotky_pianu__archeologicky_zaznam__historie__historie__typ_zmeny": ZAPSANI_AZ}
                ),
            ),
        )
    ...
    def add_ownership_lookup(self, ownership, qs=None):
        ...
        if ownership == Permissions.ownershipChoices.our:
            ...
            return (
                Q(**{"historie_zapsat_pian__in": filtered_pian_history_our})
                | (
                    Q(dokumentacni_jednotky_pianu__archeologicky_zaznam__historie__historie__typ_zmeny=ZAPSANI_AZ)
                    & Q(
                        dokumentacni_jednotky_pianu__archeologicky_zaznam__historie__historie__organizace_snapshot=self.request.user.organizace
                    )
                )
                | Q(
                    **{
                        "dokumentacni_jednotky_pianu__archeologicky_zaznam__akce__projekt__organizace": self.request.user.organizace
                    }
                )
            )
        else:
            return Q(**{"historie_zapsat_pian__in": filtered_pian_history}) | Q(
                **{"historie_zapsat_az__in": filtered_az_history}
            )
    ...
    def add_accessibility_lookup(self, permission, qs):
        ...
        pristupnost = (
            ArcheologickyZaznam.objects.filter(
                dokumentacni_jednotky_akce__pian=OuterRef("pk"), pristupnost__isnull=False
            )
        ...
        return qs.filter(
            Q(**filter) | Q(pristupnost_filter__isnull=True) | Q(self.add_ownership_lookup(permission.accessibility))
        )
    ...
    def post(self, request):
        ...
        if sheet.shape[1] != 3:
            ...
            return HttpResponseBadRequest(
                _("pian.views.importovatPianView.check.wrongColumnName") + " " + (", ".join(sheet.columns))
            )
        if sheet.shape[0] == 0:
        ...
        if not isinstance(sheet.columns[0], str) or sheet.columns[0].lower() != "label":
            logger.debug("pian.views.ImportovatPianView.post.label_check.column0", extra={"columns": sheet.columns})
            return HttpResponseBadRequest(
                _("pian.views.importovatPianView.check.wrongColumnName.Column0") + " " + sheet.columns[0]
            )
        if not isinstance(sheet.columns[1], str) or sheet.columns[1].lower() != "epsg":
            logger.debug("pian.views.ImportovatPianView.post.label_check.column1", extra={"columns": sheet.columns})
            return HttpResponseBadRequest(
                _("pian.views.importovatPianView.check.wrongColumnName.Column1") + " " + sheet.columns[1]
            )
        if not isinstance(sheet.columns[2], str) or sheet.columns[2].lower() != "geometry":
            logger.debug("pian.views.ImportovatPianView.post.label_check.column2", extra={"columns": sheet.columns})
            return HttpResponseBadRequest(
                _("pian.views.importovatPianView.check.wrongColumnName.Column2") + " " + sheet.columns[2]
            )
        try:
        ...
        if (
            ArcheologickyZaznam.objects.get(ident_cely=request.POST.get("arch_ident")).typ_zaznamu
            == ArcheologickyZaznam.TYP_ZAZNAMU_AKCE
        ):
            if request.POST.get("action", False) == "change":
                context["url"] = reverse(
                    "arch_z:update-pian",
                    args=[request.POST.get("arch_ident"), request.POST.get("dj_ident"), request.POST.get("pian_ident")],
                )
            else:
                context["url"] = reverse(
                    "arch_z:create-pian", args=[request.POST.get("arch_ident"), request.POST.get("dj_ident")]
                )
        else:
            if request.POST.get("action", False) == "change":
                context["url"] = reverse(
                    "lokalita:update-pian",
                    args=[request.POST.get("arch_ident"), request.POST.get("dj_ident"), request.POST.get("pian_ident")],
                )
            else:
                context["url"] = reverse(
                    "lokalita:create-pian", args=[request.POST.get("arch_ident"), request.POST.get("dj_ident")]
                )
