"""Jednotkové testy pro ``cron.tasks.run_data_import`` — import ArcheologickyZaznamKatastr."""
...
FILE_KEY = "az_katastry"
JOB_ID = "test-job-azk"
LOCK_TOKEN = "test-lock-token"

AZ_IDENT = "C-202300401A"


class RunDataImportArcheologickyZaznamKatastrTest(TestCase):
    """Testy ``run_data_import`` pro ``ArcheologickyZaznamKatastrMapper``."""
    ...
    def setUpTestData(cls):
        ...
        with patch(
            ...
            heslars = create_heslar_fixtures("azk")
            cls.katastr_main, _ = create_ruian_with_pian("azk", "P-0001-700001", heslars)
            cls.organizace = create_organizace("azk", heslars)
            cls.user = User.objects.create_user(  # type: ignore[attr-defined]
                email="import-runner-azk@example.cz",
                password="pass",
                is_active=True,
                organizace=cls.organizace,
                ident_cely="U-IMP-AZK-001",
                first_name="Import",
                last_name="Runner",
            )
            cls.az = create_archeologicky_zaznam(AZ_IDENT, cls.katastr_main, heslars["pristupnost"])
            ...
            okres = RuianOkres.objects.get(nazev="Okres-azk")
    ...
    def _build_payload(self, az_ident: str = AZ_IDENT, katastr_kod: int = None) -> dict:
        ...
        return FakeRedis(
            {
                ...
                f"import_data_{JOB_ID}_record_0": json.dumps(self._build_payload()),
            }
        )
    ...
    def _create_existing_azk(self) -> ArcheologickyZaznamKatastr:
        return ArcheologickyZaznamKatastr.objects.create(archeologicky_zaznam=self.az, katastr=self.katastr_other)
    ...
    def test_insert_writes_azk_to_database(self):
        """Ověřuje, že INSERT import zapíše záznam archeologicky zaznam katastr do databáze."""
        fake_redis = self._build_redis(ImportDataAdminForm.PERFORMED_ACTION_INSERT)

        self._run_import(fake_redis)

        self.assertTrue(
            ArcheologickyZaznamKatastr.objects.filter(archeologicky_zaznam=self.az, katastr=self.katastr_other).exists()
        )

    def test_update_marks_import_as_failed(self):
        # ArcheologickyZaznamKatastrMapper.allow_update = False → UPDATE skončí jako selhalý.
        """Ověřuje, že UPDATE import záznamu archeologicky zaznam katastr skončí selháním."""
        self._create_existing_azk()
    ...
    def test_delete_removes_azk_from_database(self):
        """Ověřuje, že DELETE import smaže záznam archeologicky zaznam katastr z databáze."""
        self._create_existing_azk()
    ...
    def test_database_save_failure_marks_import_as_failed(self):
        """Ověřuje, že selhání databázového uložení záznamu archeologicky zaznam katastr označí import jako selhaný."""
        fake_redis = self._build_redis(ImportDataAdminForm.PERFORMED_ACTION_INSERT)

        def failing_save(self, *args, **kwargs):
            raise RuntimeError("Selhání DB.")

        with patch.object(ArcheologickyZaznamKatastr, "save", failing_save):
            self._run_import(fake_redis)

        self._assert_import_failed(fake_redis)

    def test_history_save_failure_marks_import_as_failed(self):
        """Ověřuje, že selhání uložení historie záznamu archeologicky zaznam katastr označí import jako selhaný."""
        from core.import_data_mappers import ArcheologickyZaznamKatastrMapper
        ...
        with patch.object(
            ArcheologickyZaznamKatastrMapper, "get_record_history", staticmethod(lambda record: record)
    ...
    def test_fedora_save_failure_marks_import_as_failed(self):
        """Ověřuje, že selhání uložení metadat Fedory pro záznam archeologicky zaznam katastr označí import jako selhaný."""
        fake_redis = self._build_redis(ImportDataAdminForm.PERFORMED_ACTION_INSERT)
    ...
    def test_failure_mid_batch_rolls_back_all_inserted_records(self):
        """Ověřuje, že selhání uprostřed dávky vrátí vložené záznamy archeologicky zaznam katastr zpět."""
        first = self._build_payload()
        duplicate = self._build_payload()
    ...
    def test_user_stop_during_import_marks_status_as_stopped(self):
        """Ověřuje, že uživatelské zastavení importu záznamu archeologicky zaznam katastr nastaví stav zastaveno."""
        fake_redis = self._build_redis(ImportDataAdminForm.PERFORMED_ACTION_INSERT)
    ...
    def test_delete_of_nonexistent_record_marks_import_as_failed(self):
        """Ověřuje, že DELETE neexistujícího záznamu archeologicky zaznam katastr označí import jako selhaný."""
        fake_redis = self._build_redis(ImportDataAdminForm.PERFORMED_ACTION_DELETE)
    ...
    def test_insert_of_duplicate_pk_marks_import_as_failed(self):
        """Ověřuje, že INSERT duplicitního projekt katastr pro záznam archeologicky zaznam katastr označí import jako selhaný."""
        self._create_existing_azk()
        fake_redis = self._build_redis(ImportDataAdminForm.PERFORMED_ACTION_INSERT)
    ...
    def test_lock_lost_mid_import_sets_failed_lock_lost_status(self):
        """Ověřuje, že ztráta importního locku při importu záznamu archeologicky zaznam katastr nastaví stav failed_lock_lost."""
        fake_redis = self._build_redis(ImportDataAdminForm.PERFORMED_ACTION_INSERT)
    ...
    def test_successful_import_writes_success_marker_into_progress_details(self):
        """Ověřuje, že úspěšný import záznamu archeologicky zaznam katastr zapíše success marker do detailu průběhu."""
        fake_redis = self._build_redis(ImportDataAdminForm.PERFORMED_ACTION_INSERT)
