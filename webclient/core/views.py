...
class DownloadFile(LoginRequiredMixin, View):
    ...
    def _preprocess_image(file_content: BytesIO) -> BytesIO:
        """Zajišťuje logiku funkce ``_preprocess_image``.
        
        :param file_content: Vstupní hodnota parametru ``file_content`` použitého při zpracování. (typ: ``BytesIO``).
        :return: Návratová hodnota funkce po zpracování vstupních dat. (typ: ``BytesIO``).
        """
        return file_content
    ...
    def get(self, request, typ_vazby, ident_cely, pk, *args, **kwargs) -> FileResponse | HttpResponse:
        ...
        :return: Návratová hodnota funkce po zpracování vstupních dat. (typ: ``FileResponse | HttpResponse``).
        """
        try:
            check_soubor_vazba(typ_vazby, ident_cely, pk)
...
class ExportMixinDate(ExportMixin):
    ...
    def get_export_filename(self, export_format, export_name=None):
        """Zajišťuje logiku funkce ``get_export_filename``.
        
        :param export_format: Vstupní hodnota parametru ``export_format`` použitého při zpracování.
        :param export_name: Vstupní hodnota parametru ``export_name`` použitého při zpracování.
        :return: Návratová hodnota funkce po zpracování vstupních dat.
        """
...
class PermissionFilterMixin:
    ...
    def check_filter_permission(self, qs, action=None):
        """Zajišťuje logiku funkce ``check_filter_permission``.
    ...
    def add_status_lookup(self, permission):
        """Zajišťuje logiku funkce ``add_status_lookup``.
...
class SearchListView(ExportMixin, LoginRequiredMixin, SingleTableMixin, FilterView, PermissionFilterMixin):
    ...
    def create_export(self, export_format):
        """Zajišťuje logiku funkce ``create_export``.
        ...
        def check_if_aborted(r_inner: Redis, key_inner: str):
            """Zajišťuje logiku funkce ``check_if_aborted``.
            
            :param r_inner: Vstupní hodnota parametru ``r_inner`` použitého při zpracování. (typ: ``Redis``).
            :param key_inner: Vstupní hodnota parametru ``key_inner`` použitého při zpracování. (typ: ``str``).
            :return: Návratová hodnota funkce po zpracování vstupních dat.
            """
        ...
        def update_progress_bar(r_inner: Redis, key_inner: str, new_value: int, message: str):
            """Zajišťuje logiku funkce ``update_progress_bar``.
            
            :param r_inner: Vstupní hodnota parametru ``r_inner`` použitého při zpracování. (typ: ``Redis``).
            :param key_inner: Vstupní hodnota parametru ``key_inner`` použitého při zpracování. (typ: ``str``).
            :param new_value: Vstupní hodnota parametru ``new_value`` použitého při zpracování. (typ: ``int``).
            :param message: Vstupní hodnota parametru ``message`` použitého při zpracování. (typ: ``str``).
            :return: Návratová hodnota funkce po zpracování vstupních dat.
            """
        ...
        def file_iterator(content, r, redis_variable_name, chunk_size=8192):
            """Zajišťuje logiku funkce ``file_iterator``.
            
            :param content: Vstupní hodnota parametru ``content`` použitého při zpracování.
            :param r: Vstupní hodnota parametru ``r`` použitého při zpracování.
            :param redis_variable_name: Vstupní hodnota parametru ``redis_variable_name`` použitého při zpracování.
    ...
    def _get_sort_params(self):
        """Zajišťuje logiku funkce ``_get_sort_params``.
...
class StahnoutDataHistorickaView(LoginRequiredMixin, View):
    ...
    def get(self, request, model_name, ident_cely, timestamp):
        ...
        def context_processor(content):
            """Zajišťuje logiku funkce ``context_processor``.
...
def check_soubor_vazba(typ_vazby, ident, id_zaznamu):
    """Zajišťuje logiku funkce ``check_soubor_vazba``.
    
    :param typ_vazby: Vstupní hodnota parametru ``typ_vazby`` použitého při zpracování.
    :param ident: Vstupní hodnota parametru ``ident`` použitého při zpracování.
    :param id_zaznamu: Vstupní hodnota parametru ``id_zaznamu`` použitého při zpracování.
    :return: Návratová hodnota funkce po zpracování vstupních dat.
    """
...
class TranslationImportView(FormView, RosettaFileLevelMixinWithBackup):
    ...
    def handle_uploaded_file(self, f):
        """Zajišťuje logiku funkce ``handle_uploaded_file``.
        
        :param f: Vstupní hodnota parametru ``f`` použitého při zpracování.
        :return: Návratová hodnota funkce po zpracování vstupních dat.
        """
