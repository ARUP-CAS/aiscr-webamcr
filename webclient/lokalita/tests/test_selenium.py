...
class AkceLokality(BaseSeleniumTestClass):
    ...
    def test_143_test_Fedory_lokalita_p_001(self):
        ...
        time = self.getTime()
        # id_hlavni_katastr
        self.select_dynamic_select2_autocomplete_option("id_hlavni_katastr", "Lanšperk")
        self.select_dynamic_selectpicker_option("id_druh", "pol")
        ...
        self.select_nth_selectpicker_option("dj_typ_id")
        with WaitForPageLoad(self.driver):
            self.ElementClick(By.ID, "newDjSubmitButton")
        ...
        self.select_nth_selectpicker_option("dj_typ_id", 1)
        with WaitForPageLoad(self.driver):
        ...
        self.wait(0.5)
        self.select_nth_selectpicker_option("id_presnost")
        with WaitForPageLoad(self.driver):
        ...
        self.select_dynamic_selectpicker_option("id_obdobi", "únětická kultura", wait_ajax=True)
        self.select_dynamic_selectpicker_option("id_areal", "poh")
        with WaitForPageLoad(self.driver):
            self.ElementClick(By.ID, "createCompotSubmitButton")
        ...
        # Úprava komponenty
        time = self.getTime()
        self.select_dynamic_selectpicker_option(f"id_{ident}-K001-obdobi", "moustérien")
        self.select_nth_selectpicker_option(f"id_{ident}-K001-jistota", 1)
        self.ElementSendKeys(By.ID, f"id_{ident}-K001-presna_datace", "ne")
        self.select_dynamic_selectpicker_option(f"id_{ident}-K001-areal", "osídlený skalní prostor", wait_ajax=True)
        self.select_multiple_selectpicker_options(f"id_{ident}-K001-aktivity", ["pohřbívání", "sídlení"])
        with WaitForPageLoad(self.driver):
        ...
        self.ElementSendKeys(By.ID, f"id_{ident}-K001_o-0-pocet", "1")
        ...
        self.select_dynamic_selectpicker_option(f"id_{ident}-K001_p-0-specifikace", "kámen – štípaný")
        self.select_dynamic_selectpicker_option(f"id_{ident}-K001_o-0-specifikace", "konstrukce – dřevěná")
        ...
        self.select_dynamic_selectpicker_option("id_typ_dokumentu", "nálezová zpráva", wait_ajax=True)
        ...
        self.select_multiple_select2_autocomplete_options("id_dokument", ["M-TX-194300151"])
        with WaitForPageLoad(self.driver):
            self.ElementClick(By.ID, "submit-btn")
