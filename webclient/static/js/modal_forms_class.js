...
class Modal {
    ...
    getForm() {
        ...
        $(this.modalIDD).find(this.modalContent).load(this.formURL, function (response, status, xhr) {
            if (xhr.status == "403") {
                ...
                $(object.modalIDD).off('show.bs.modal').on('show.bs.modal', function () {
                ...
                $(object.modalIDD).off('shown.bs.modal').on('shown.bs.modal', function () {
                    object.resetScripts();
                });
    ...
    resetScripts() {
        ...
        $sp.filter(':not([multiple])')
            ...
            if (sp && !sel.multiple) {
                var val = sel.value;
                sp.selectpicker.main.data.forEach(function (item) {
                    if (item.type === 'option' && item.selected && String(item.value) !== String(val)) {
                        item.selected = false;
