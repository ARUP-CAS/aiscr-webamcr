...
class AnnouncementForm(forms.Form):
    ...
    person = forms.CharField(
        label=_("Osoba realizující stavební či jiný záměr"),
        max_length=100,
        required=True,
    )
    # Time schedule part
    expected_start_date = forms.DateField(
        label=_("Předpokládaný datum zahájení prací"), required=True
    )
    expected_end_date = forms.DateField(
        label=_("Předpokládaný datum ukončení prací"), required=True
    )
