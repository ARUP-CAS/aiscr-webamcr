...
class Mailer:
    ...
    def _notification_should_be_sent(
        ...
        if notification_type.ident_cely in groups.keys():
            try:
                ...
                logger.debug(
                    "services.mailer._notification_should_be_sent.group_not_found",
                    extra={
                        "user": user,
        ...
        if notification_type.ident_cely in always_active:
            ...
            notification_is_enabled = user.notification_types.filter(
                pk=group_key
            ).first()
    ...
    def _notification_was_sent(
        ...
        notification_log = user.notification_log.filter(
    ...
    def _log_notification(
        cls, notification_type: "uzivatel.models.UserNotificationType", receiver_object
    ):
        log = uzivatel.models.NotificationsLog(
            notification_type=notification_type, content_object=receiver_object
        )
        log.save()
        logger.debug(
    ...
    def send(
        ...
        if "@" in to:
            ...
            logger.info(
                ...
                extra={"from_email": from_email, "to": to, "subject": subject},
            )
            if attachment_path:
            ...
            logger.warning(
                "services.mailer.send.invalid_email",
    ...
    def send_eu02(cls, user: "uzivatel.models.User"):
        ...
        notification_type = uzivatel.models.UserNotificationType.objects.get(
    ...
    def send_eu03(cls, user: "uzivatel.models.User"):
        ...
        if not cls._notification_was_sent(notification_type, user):
            ...
            cls._log_notification(
                notification_type=notification_type, receiver_object=user
            )
    ...
    def send_eu04(cls, user: "uzivatel.models.User"):
        ...
        for superuser in superusers:
            if Mailer._notification_should_be_sent(
                notification_type=notification_type, user=superuser
            ):
    ...
    def send_eu06(cls, user: "uzivatel.models.User", groups):
        ...
        if groups[0] is not None:
            logger.debug(
                "services.mailer.send_eu06.groups",
                extra={"ident_cely": IDENT_CELY, "groups": groups},
            )
    ...
    def _send_notification_for_project(cls, project, notification_type):
        ...
        if cls._notification_should_be_sent(
            notification_type=notification_type, user=user
        ):
            if not cls._notification_was_sent(notification_type, user):
                subject = notification_type.predmet.format(
    ...
    def _send_notification_for_projects(cls, projects, notification_type):
        ...
        with ThreadPoolExecutor() as executor:
            executor.map(
                Mailer._send_notification_for_project,
                projects,
                repeat(notification_type),
            )
    ...
    def send_enz01(cls):
        ...
        projects = projekt.models.Projekt.objects.filter(
            stav=PROJEKT_STAV_UKONCENY_V_TERENU, termin_odevzdani_nz=in_90_days
        )
    ...
    def send_enz02(cls):
        ...
        projects = projekt.models.Projekt.objects.filter(
            stav=PROJEKT_STAV_UKONCENY_V_TERENU, termin_odevzdani_nz__lt=today
        )
        cls._send_notification_for_projects(projects, notification_type)
    ...
    def send_ev01(cls, arch_z: "arch_z.models.ArcheologickyZaznam", reason):
        ...
        first_log_entry = (
            Historie.objects.filter(vazba=arch_z.historie)
        ...
        if Mailer._notification_should_be_sent(
            ...
            cls.send(
                subject=subject, to=first_log_entry.uzivatel.email, html_content=html
            )
    ...
    def _send_a(
        ...
        if isinstance(obj, projekt.models.Projekt):
            ...
            history_log = Historie.objects.filter(
                vazba__archeologickyzaznam__ident_cely=obj.ident_cely,
    ...
    def _send_ep01(cls, project, notification_type):
        ...
        project_files = list(
            projekt.models.Soubor.objects.filter(
                vazba=project.soubory.id,
    ...
    def _send_ep03(cls, project, notification_type):
        ...
        first_log_entry = (
            Historie.objects.filter(vazba=project.historie)
            .order_by("datum_zmeny")
            .first()
        )
    ...
    def send_ep04(cls, project: "projekt.models.Projekt", reason):
        ...
        history_log = Historie.objects.filter(
            vazba__projekt_historie__ident_cely=project.ident_cely,
    ...
    def send_ep05(cls, project: "projekt.models.Projekt"):
        IDENT_CELY = "E-P-05"
        logger.debug("services.mailer.send_ep05", extra={"ident_cely": IDENT_CELY})
        notification_type = uzivatel.models.UserNotificationType.objects.get(
            ident_cely=IDENT_CELY
        )
        subject = notification_type.predmet.format(ident_cely=project.ident_cely)
    ...
    def send_en03_en04(cls, samostatnyNalez: "pas.models.SamostatnyNalez", reason):
        ...
        log = (
            Historie.objects.filter(
                vazba__sn_historie__ident_cely=samostatnyNalez.ident_cely,
                typ_zmeny=look_for_stav,
            )
            .order_by("datum_zmeny")
            .first()
        )
    ...
    def send_en06(cls, cooperation: "pas.models.UzivatelSpoluprace"):
        ...
        if Mailer._notification_should_be_sent(
            notification_type=notification_type, user=cooperation.spolupracovnik
        ):
            cls.send(
                subject=subject, to=cooperation.spolupracovnik.email, html_content=html
            )
    ...
    def send_ek01(cls, document: "dokument.models.Dokument"):
        ...
        first_log_entry = Historie.objects.filter(
            vazba=document.historie, typ_zmeny=ZAPSANI_DOK
        ).first()
        if first_log_entry:
            if cls._notification_should_be_sent(
                notification_type=notification_type, user=first_log_entry.uzivatel
