...
- Monitoring and logging stack (Elasticsearch, Kibana, Logstash, Prometheus)
...
AI agents must always target **`test`** unless explicitly instructed otherwise.
...
Agents should treat these files as the **persistent state of the review process**.
...
Large architectural changes should only be proposed, not performed automatically.
...
When verifying behaviour or documentation, the following priority applies:
...
- Django behaviour should be verified against official Django documentation
- Docker image metadata should be verified against container registries
- Elasticsearch and Redis behaviour should be verified against upstream documentation
...
Generated documentation blocks must not be edited manually if scripts exist.
...
The `docs_agents` directory stores the persistent state
...
These repositories share similar governance and AI-assisted review structure.

Agents working across repositories should preserve consistency
...
3. Do not manually edit `*/migrations/*.py` unless a schema change is required.
...
The `method-docstring-style-reminder` hook is non-blocking,
but warnings should be treated seriously.
...
2. After changing Selenium tests or module structure, run the generators.
3. After changing dependencies, verify whether dependency documentation must be regenerated.
...
If `selenium_testy.rst` is generated by a script, verify consistency with
`docs/generate_selenium_test_docs.py`.
...
Certificates in `cert/` are self-signed for development only
