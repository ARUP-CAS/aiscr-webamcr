...
### Saving reviews (fixed rule)

Every review performed by an agent must be saved to the standardized target
directory **`.agents/reports/review_reports/`**. Do not scatter reviews into
other directories (e.g. `.agents/reviews/`, `.agents/analysis/`, or the top
level of `.agents/reports/`).

A review must always be saved as a **Markdown** document (`.md`). The filename
must include a **random set of characters** (a random suffix) so that one agent
cannot overwrite a review created by another agent. Use the pattern:

`review_<topic>_<random>.md`

where `<random>` is a short random alphanumeric token (for example 6–8
characters, such as `a3f9k2`). Example:
`review_validation_refactor_a3f9k2.md`.

Every review document must also state the **model name** that produced it (for
example the exact model identifier such as `claude-opus-4-8`). Record it inside
the document — for example in a header line or metadata block at the top — so it
is always clear which model generated the review.

### Language of agent-generated Markdown (fixed rule)

All Markdown documents generated by agents — plans, analysis reports, reviews,
and any similar output — must be written **in English**, regardless of the
language used elsewhere in the repository (e.g. Czech docstrings or UI). This
extends the planning-phase English rule below to every agent-produced Markdown
